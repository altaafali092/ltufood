...
class CartService
{
    ...
    public function removeItemFromCart(FoodItem $foodItem): void
    {
        $items = $this->all();
        unset($items[$foodItem->id]);
        ...
        $this->put($items);
            ...
            // if (Auth::check()) {
            //     $this->removeCartItemFromDatabase($foodItem);
            // } else {
            //     $this->removeCartItemFromCookies($foodItem);
            // }
    ...
    /**
     * @param  array<int, array{food_item_id: int, title: string, slug: string|null, price: float, quantity: int}>  $items
     */
